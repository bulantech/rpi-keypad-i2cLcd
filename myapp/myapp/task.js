...
const skepSerialNotFound = config.get('skepSerialNotFound');
...
const checkKey = (col) => {
	...
	switch(gState) {
		...
		case 'init-serial':
			switch(key) {
				case '*': return task('init-rfid-0'); break;
				case '#': if(skepSerialNotFound) return task('ready'); break;
			}
		break
		case 'card-id':
			switch(key) {
	...
	switch(command) {
		...
		case 'sale_by_member_debit_money':
			...
			switch(gState) {
				...
				break
				case 'fill-litres':
					bathText = ( Math.floor(litresText*price) +'').padStart(4, '0')
